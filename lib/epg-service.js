...
// EPG.PW's country XMLTV currently exports its France schedule eight hours ahead
// of the ISO offset declared in the file. Its per-channel JSON endpoint confirms
// the real French broadcast time, so the correction belongs to this source only.
const DEFAULT_SOURCES = [{
  id: 'epgpw-fr', name: 'EPG.PW France', url: 'https://epg.pw/xmltv/epg_FR.xml.gz',
  compressed: true, timeShiftMs: -8 * 3600000
}];
const DEFAULT_ALIASES = {
  ...
  'bein max 4 france': 'bein sports max 4', 'bein max 5 france': 'bein sports max 5',
  'bein max 6 france': 'bein sports max 6', 'bein max 7 france': 'bein sports max 7',
  'bein max 8 france': 'bein sports max 8', 'bein max 9 france': 'bein sports max 9',
  'canal plus 360': 'canal plus sport 360', 'canal plus foot': 'canal plus foot',
  'canal plus sport': 'canal plus sport', 'canal plus docs': 'canal plus docs',
  'rmc decouverte': 'rmc découverte', 'equipe 21': "l'équipe", 'l equipe': "l'équipe"
};
...
function programProgress(program, now = Date.now()) {
  const start = program?.start instanceof Date ? program.start.getTime() : new Date(program?.start || 0).getTime();
  const end = program?.end instanceof Date ? program.end.getTime() : new Date(program?.end || 0).getTime();
  if (!Number.isFinite(start) || !Number.isFinite(end) || end <= start || now < start || now >= end) return 0;
  return Math.max(0, Math.min(100, Math.round(((now - start) / (end - start)) * 100)));
}

function parseXmltvStream(stream, { now = Date.now(), maxPrograms = 250000, timeShiftMs = 0 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    parser.on('opentag', node => {
      ...
      if (node.name === 'programme') {
        ...
        const end = parseXmltvDate(node.attributes.stop);
        currentProgram = {
          channelId: node.attributes.channel || '',
          start: start && timeShiftMs ? new Date(start.getTime() + timeShiftMs) : start,
          end: end && timeShiftMs ? new Date(end.getTime() + timeShiftMs) : end,
...
class EpgService {
  ...
  async loadSources() {
    ...
    for (const source of this.sources) {
      try {
        ...
        const parsed = await parseXmltvStream(stream, { timeShiftMs: Number(source.timeShiftMs) || 0 });
  ...
  async guideById(id, limit = 24) {
    const snapshot = await this.refresh();
    const channel = snapshot.channels.get(String(id));
    if (!channel) return null;
    const now = Date.now();
    const programs = (snapshot.programs.get(channel.id) || []).filter(item => item.end.getTime() >= now).slice(0, limit);
    return { channel: channel.names[0] || channel.id, channelId: channel.id, programs, source: snapshot.source, updatedAt: snapshot.updatedAt, stale: snapshot.expires <= now };
  }

  async channelList() {
    const snapshot = await this.refresh();
    return {
      channels: [...snapshot.channels.values()].map(channel => ({ id: channel.id, name: channel.names[0] || channel.id, names: channel.names, icon: channel.icon || '' })),
  ...
  status() {
    return {
      available: Boolean(this.snapshot.channels.size),
      refreshing: Boolean(this.pending),
      source: this.snapshot.source || this.sources[0]?.name || '',
      sourceUrl: this.sources[0]?.url || '',
      updatedAt: this.snapshot.updatedAt || null,
      expiresAt: this.snapshot.expires ? new Date(this.snapshot.expires).toISOString() : null,
      channelCount: this.snapshot.channels.size,
      programCount: this.snapshot.programCount,
...
module.exports = { DEFAULT_SOURCES, EpgService, normalizeChannelName, parseXmltvDate, parseXmltvStream, programProgress };
